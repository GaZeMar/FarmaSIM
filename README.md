...
No subas el archivo ZIP como un único archivo: descomprímelo y sube el contenido del proyecto,
incluidas las carpetas `src` y `.github` y el archivo `package.json`.

Desde PowerShell, dentro de la carpeta descomprimida:

```powershell
git init
git add .
git commit -m "Publicar PharmacoSim"
git branch -M main
git remote add origin https://github.com/<tu-usuario>/<nombre-del-repositorio>.git
git push -u origin main
```

El repositorio de GitHub debe crearse vacío (sin README, licencia ni `.gitignore`) antes de
ejecutar estos comandos. Después del `push`, activa **Settings → Pages → Source: GitHub Actions**.

---

## 💻 Ejecución y Desarrollo Local

> **Importante:** no abras `index.html` con doble clic. Este es un proyecto React + TypeScript
> y el navegador no puede ejecutar directamente el archivo `src/main.tsx`; primero necesita
> que Vite lo transforme y lo sirva mediante HTTP.

Instala primero [Node.js LTS](https://nodejs.org/) si `node --version` y `npm --version` no
funcionan en PowerShell.
...
Si descargaste el ZIP en vez de clonar el repositorio, descomprímelo, abre PowerShell dentro
de esa carpeta y ejecuta solamente:
